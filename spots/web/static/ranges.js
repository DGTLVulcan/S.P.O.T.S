...
(function () {
  ...
  function clamp() {
    // Keep at least a corner of the map on screen, so it can't be flung
    // out of view and lost with no way back but Fit.
    const box = stage.getBoundingClientRect();
    const width = image.naturalWidth * scale;
    const height = image.naturalHeight * scale;
    const slackX = Math.max(0, (width - box.width) / 2) + box.width * 0.4;
    const slackY = Math.max(0, (height - box.height) / 2) + box.height * 0.4;
  ...
  function fit() {
    const box = stage.getBoundingClientRect();
    if (!image.naturalWidth || !box.width) return;
    fitScale = Math.min(box.width / image.naturalWidth, box.height / image.naturalHeight);
    scale = fitScale;
    x = 0;
    y = 0;
    apply();
  ...
  else image.addEventListener("load", fit);
